...
public class CourseEnrollment {
    ...
    public boolean isTrialActive() {
        boolean active = Boolean.TRUE.equals(trialUsed) &&
                trialEndDate != null &&
    ...
    public boolean canAccessContent() {
        boolean hasAccess = Boolean.TRUE.equals(isEnrolled) &&
                (isTrialActive() || hasActiveSubscription());

        // Debug logging
        if (!hasAccess && Boolean.TRUE.equals(isEnrolled)) {
            if (!isTrialActive() && !hasActiveSubscription()) {
                System.out.println("Access denied: Trial expired and no active subscription for enrollment " + id);
            }
        }
    ...
    public double getEffectiveOutstandingBalance() {
        // If subscription is active and not expired, no balance
        if (hasActiveSubscription()) {
            return 0.0;
        }

        // If trial is active, no balance yet
        if (isTrialActive()) {
            return 0.0;
        }

        // If trial expired, return outstanding balance or course price
    ...
    public boolean needsPayment() {
        return !isTrialActive() && !hasActiveSubscription() && getEffectiveOutstandingBalance() > 0;
    }
